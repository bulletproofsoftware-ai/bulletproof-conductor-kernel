...
- **Memory egress: NONE.** `claude-memory-mcp` writes to a local Qdrant instance (default `http://localhost:6333`, override with `QDRANT_URL`). There is no network egress under default configuration. Operators who reconfigure Qdrant to a remote instance are responsible for the resulting egress.
...
- **Bundled dependency license audit.** F-20 / OH-4. v0.1.0 ships under Apache-2.0 with zero direct dependencies. Phase 8 conducts a license audit before public release; any GPL/AGPL contamination would force a relicense decision.
